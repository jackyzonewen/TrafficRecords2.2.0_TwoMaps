...
- (UIView *)snapShotFromRowAtMovingIndexPath
{
    ...
    if ([touchedCell respondsToSelector:@selector(snapshotViewAfterScreenUpdates:)]) {
        ...
        UIImage *image = [self imageOfView:touchedCell];
...
- (void)determineAutoscrollDistanceForSnapShot
{
    ...
    if ([self canScroll] && CGRectIntersectsRect(self.snapShotOfMovingCell.frame, self.bounds))
	{
        ...
        CGFloat distanceToTopEdge = touchLocation.y - CGRectGetMinY(self.bounds) - topHiddenH;
