...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    if (downloadDelegates.count==0) {
        ...
        NSString*startLoadTime=[userDefault objectForKey:@"startLoadTime"];
        NSString*beginLoadTime= [userDefault objectForKey:@"beginLoadTime"];
        NSString*firstOpen= [userDefault objectForKey:@"firstOpen"];
        if (!beginLoadTime&&firstOpen&&startLoadTime) {
            NSString*networkType=[userDefault objectForKey:@"NetworkType"];
            if (networkType) {
                if (![networkType isEqualToString:@"0"]) {
                    NSString*eventId=@"";
                    NSString*keyName=@"";
                    if ([networkType isEqualToString:@"3G"]) {
                        eventId=@"3G_firstViewLoadTimeNew";
                        keyName=@"3gtimeNew";
                    }
                    if ([networkType isEqualToString:@"WiFi"]) {
                        eventId=@"wifi_firstViewLoadTimeNew";
                        keyName=@"wifitimeNew";
                    }
                    [userDefault setObject:@"YES" forKey:@"beginLoadTime"];
                    [userDefault removeObjectForKey:@"firstOpen"];
                    // NSLog(@"-----------------------------endLoadTime-------------------------------%@",[NSDate date ]);
                }
            }
        }
    }
